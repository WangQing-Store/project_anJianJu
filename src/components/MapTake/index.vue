  ...
  <div>
    <div class="disflex">
      <el-form-item label="经度" prop="longitude">
        <el-input :readonly="isminingReadonly" v-model="longitudes" @change="setLongitude" />
      </el-form-item>
      <el-form-item label="纬度" prop="latitude">
        <div class="disflex">
          <el-input style="margin-right:10px" :readonly="isminingReadonly" v-model="latitudes" @change="setLatitude" />
          <el-button type="primary" @click="handleLngLat" :disabled="isminingReadonly">地图取点</el-button>
        </div>
      </el-form-item>
    </div>
    <!--坐标选取-->
    <el-dialog :visible.sync="dialogVisible" title="选择坐标">
      <div>
        <el-input placeholder="请输入内容" ref="searchText" @input="initSearch" id="tipinput" v-model="searchKey" style="width:300px"></el-input>
      ...
      <el-row>
        <div class="amap-page-container">
          <el-amap :zoom="zoom" :center="center" :events="events" vid="vamap" :plugin="plugin">
            <el-amap-marker v-for="(marker,index) in markers" :key="index" :position="center" :events="marker.events"></el-amap-marker>
  ...
  methods: {
    ...
    getLocation () {
      ...
      AMap.event.addListener(geolocation, "complete", result => {
        _that.point(result.position.lng, result.position.lat, result.formattedAddress);
    ...
    point (lng, lat, text) {
      ...
      formatted.forEach((item, index) => {
        ...
        windows.push({
          ...
          address: `<div style="background-color:#fff;border: 1px solid #fff; border-radius: 5px;padding:5px">${item.text}</div>`
        })
      })
      //  加点
    ...
    lngLatSubmit () {
      ...
      this.$emit("setLongitudeLatitude", this.lng, this.lat);
